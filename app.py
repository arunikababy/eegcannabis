...
def build_all_conditions_paired_effect_figure(
    ...
    """Build an offset dot plot of Cohen's dz across conditions and channels."""
    figure = go.Figure()
    channel_positions = {
        channel: index
    ...
    for comparison in ALL_CONDITIONS_COMPARISON_ORDER:
        ...
        comparison_data["Channel_Order"] = comparison_data["channel"].map(
            channel_positions
        )
        comparison_data = comparison_data.sort_values("Channel_Order")
        y_values = comparison_data["channel"].map(channel_positions).astype(float)
        y_values = y_values + comparison_offsets[comparison]
        ...
        figure.add_trace(
            go.Scatter(
                ...
                hovertemplate=(
                    ...
                    "Channel: %{customdata[0]}<br>"
                    "Cohen's dz: %{x:+.3f}<br>"
                    "Effect: %{customdata[6]}<br>"
                    "Mean change: %{customdata[1]:+,.4g}<br>"
                    "95% CI for mean change: [%{customdata[2]:,.4g}, "
                    "%{customdata[3]:,.4g}]<br>"
                    "Raw p: %{customdata[4]:.4f}<br>"
                    "FDR q: %{customdata[5]:.4f}<extra></extra>"
                ),
            )
        )

    maximum_effect = max(
        1.2, float(selected_data["cohens_dz"].abs().max()) * 1.22
    )
    ...
    figure.add_vline(x=0, line_width=1.8, line_color="#5F6F66")
    for position in channel_positions.values():
        figure.add_hline(
            y=position,
            line_width=1,
            line_color="#E4ECE7",
    ...
    figure.update_layout(
        title=(
            "<b>Cross-Condition Effect-Size Profile</b><br>"
            f"<sup>{selected_band} · {selected_feature} · {test_label}</sup>"
        ),
        xaxis=dict(
            title="Cohen's dz (negative = lower after; positive = higher after)",
            range=[-maximum_effect, maximum_effect],
        ),
        yaxis=dict(
            title="EEG Channel",
            tickmode="array",
            tickvals=list(channel_positions.values()),
            ticktext=list(channel_positions.keys()),
            range=[len(channel_positions) - 0.45, -0.45],
        ),
        hovermode="closest",
    )
    return style_all_conditions_figure(figure, 500)
...
def render_all_conditions_paired_statistics():
    ...
    st.caption(
        "Points show Cohen's dz estimates; the offset separates comparisons at "
        "each channel. Vertical guides mark conventional |dz| thresholds of "
        "0.2, 0.5, and 0.8. Confidence intervals in hover labels apply to mean "
...
def render_subject_10_topomap_brain():
    """Render the landing page for scalp-level EEG topomap visualizations."""
    st.subheader("Topomap Brain")
    st.caption(
        "Explore the spatial distribution of EEG spectral power across the "
        "TP9, AF7, AF8, and TP10 scalp channels."
    )

    with st.container(border=True):
        st.info(
            "A topomap summarizes how an EEG feature is distributed across "
            "the measured scalp locations. It is a scalp-level visualization, "
            "not a map of neural-source localization inside the brain."
        )
        st.write(
            "This page is reserved for condition-level and paired-difference "
            "topomaps across the Alpha, Beta, Delta, Gamma, and Theta bands."
        )

    with st.expander("Planned topomap views", expanded=True):
        st.markdown(
            """
            - Mean spectral-power topomap for each condition
            - Paired difference topomap with a zero-centered color scale
            - Frequency-band and feature selection
            - Brief interpretation and statistical context
            """
        )
