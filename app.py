...
def build_bcf_acf_similarity_tradeoff_figure(
    selected_data,
    selected_band,
    selected_condition,
):
    """Plot Correlation against RMSE while encoding DTW as marker size."""
    figure = go.Figure()
    condition_order = [
        condition
        for condition in ["BCF", "ACF"]
        if condition in set(selected_data["Condition"])
    ]
    symbols = {"BCF": "circle", "ACF": "diamond"}
    dtw_min = selected_data["DTW"].min()
    dtw_range = selected_data["DTW"].max() - dtw_min
    ...
    for channel in BCF_ACF_CHANNEL_ORDER:
        for condition in condition_order:
            ...
            if trace_data.empty:
                continue
            marker_sizes = 12 + 17 * (
                (trace_data["DTW"] - dtw_min)
                / (dtw_range if dtw_range else 1)
            )
            trace_name = (
                f"{channel} · {condition}"
                if selected_condition == "All Conditions"
            ...
            figure.add_trace(
                go.Scatter(
                    ...
                    customdata=trace_data[
                        ["Feature_Display", "Condition", "Channel", "DTW"]
                    ].values,
                    hovertemplate=(
                        "Feature: %{customdata[0]}<br>"
                        "Condition: %{customdata[1]}<br>"
                        "Channel: %{customdata[2]}<br>"
                        "Correlation: %{y:.4f}<br>RMSE: %{x:.4f}<br>"
                        "DTW: %{customdata[3]:.5f}<extra></extra>"
                    ),
                )
            )
    ...
    figure.update_layout(
        title=(
            "Correlation-RMSE Similarity Trade-off<br>"
            f"<sup>{selected_band} · {selected_condition}; marker size = DTW</sup>"
        ),
        xaxis_title="RMSE (lower is closer)",
        yaxis_title="Correlation (higher is stronger)",
        legend={
            "orientation": "h",
            "yanchor": "bottom",
            "y": 1.02,
    ...
    correlation_min = selected_data["Correlation"].min()
    figure.update_yaxes(range=[max(-1, correlation_min - 0.08), 1.02])
    return style_bcf_acf_pre_post_figure(figure, 540)
...
def build_bcf_acf_similarity_band_overview(dataframe):
...
def render_bcf_acf_quantitative_similarity():
    ...
    tradeoff_figure = build_bcf_acf_similarity_tradeoff_figure(
        selected_data,
        selected_band,
        selected_condition,
    )
    st.plotly_chart(
        tradeoff_figure,
        use_container_width=True,
        key="bcf_acf_similarity_tradeoff",
    )
    st.caption(
        "Points toward the upper-left combine stronger correlation with lower "
        "RMSE. Larger markers indicate larger DTW distance."
    )
