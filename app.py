...
def get_bcf_acf_pre_post_interpretation(
    selected, feature_data, band, feature, channel
):
    """Create a concise interpretation for the four selected visualizations."""
    total = len(selected)
    increased = int((selected["Direction"] == "Increase").sum())
    decreased = int((selected["Direction"] == "Decrease").sum())
    unchanged = int((selected["Direction"] == "No Change").sum())
    ...
    if increased > decreased and increased > unchanged:
        local_pattern = (
            f"{increased} of {total} subjects increased from BCF to ACF"
        )
    elif decreased > increased and decreased > unchanged:
        local_pattern = (
            f"{decreased} of {total} subjects decreased from BCF to ACF"
        )
    else:
        local_pattern = "the paired subjects showed a mixed direction"

    channel_means = feature_data.groupby("Channel")["Change"].mean()
    strongest_channel = channel_means.abs().idxmax()
    strongest_mean = channel_means.loc[strongest_channel]

    if mean_change * median_change < 0:
        distribution_note = (
            "The mean and median have opposite signs, indicating that one or "
            "more relatively large observations influenced the average."
        )
    else:
        distribution_note = (
            "The mean and median point in the same direction, although the "
            "individual trajectories still show subject-level variability."
        )

    return (
        f"For {feature} at {channel} in {band}, {local_pattern}. The mean "
        f"change was {format_bcf_acf_change(mean_change)} and the median change "
        f"was {format_bcf_acf_change(median_change)}. {distribution_note} "
        f"Across channels, {strongest_channel} had the largest absolute mean "
        f"change ({format_bcf_acf_change(strongest_mean)}). These plots are "
        "descriptive and do not establish statistical significance."
    )


def build_bcf_acf_pre_post_paired_figure(
    selected, band, feature, channel
):
    """Build the paired BCF-to-ACF subject trajectory plot."""
    paired_data = selected.melt(
        id_vars=["Subject"],
        value_vars=["BCF", "ACF"],
        var_name="Condition",
    ...
    paired_data["Subject Label"] = (
        "Subject " + paired_data["Subject"].astype(str)
    )
    figure = px.line(
        paired_data,
        x="Condition",
        ...
        title=(
            "Paired Before-After Plot<br>"
            f"<sup>{band} | {feature} | {channel}</sup>"
        ),
    )
...
def build_bcf_acf_change_distribution_figure(
    selected, band, feature, channel
):
    """Build a histogram and rug plot of within-subject change values."""
    plot_data = selected.copy()
    figure = px.histogram(
        plot_data,
        x="Change",
        color="Direction",
        nbins=min(8, max(4, len(plot_data) // 2)),
        marginal="rug",
        opacity=0.78,
        barmode="overlay",
        color_discrete_map=BCF_ACF_DIRECTION_COLORS,
        ...
        title=(
            "Distribution of Within-Subject Change<br>"
            f"<sup>{band} | {feature} | {channel}</sup>"
        ),
    )
    figure.add_vline(x=0, line_color="#374151", line_dash="dash")
    figure.update_layout(
        height=440,
        margin=dict(l=20, r=20, t=75, b=20),
        xaxis_title="Change (ACF - BCF)",
...
def build_bcf_acf_mean_change_figure(feature_data, band, feature):
    """Build a channel-level mean-change chart for one feature."""
    summary = (
    ...
    figure.add_trace(
        go.Bar(
            x=summary["Channel"],
            y=summary["Mean_Change"],
            marker_color=bar_colors,
            text=[format_bcf_acf_change(value) for value in summary["Mean_Change"]],
            textposition="outside",
            customdata=summary[["Median_Change", "Subjects"]].values,
            hovertemplate=(
                "Channel: %{x}<br>Mean change: %{y:.4g}<br>"
    ...
    figure.add_trace(
        go.Scatter(
            x=summary["Channel"],
            y=summary["Median_Change"],
            mode="markers",
            marker={"symbol": "diamond", "size": 11, "color": "#111827"},
            hovertemplate=(
                "Channel: %{x}<br>Median change: %{y:.4g}<extra></extra>"
            ),
            name="Median change",
        )
    )
    figure.add_hline(y=0, line_color="#374151", line_width=1)
    figure.update_layout(
        title=(
...
def build_bcf_acf_direction_by_channel_figure(feature_data, band, feature):
    """Build a 100% stacked direction-of-change chart by channel."""
    direction_order = ["Increase", "Decrease", "No Change"]
    ...
    figure.update_traces(
        ...
        textposition="inside",
        hovertemplate=(
            "Channel: %{x}<br>Direction: %{fullData.name}<br>"
            "Subjects: %{customdata[0]:.0f}<br>Percentage: %{y:.1f}%"
            "<extra></extra>"
        ),
    )
    figure.update_layout(
...
def render_bcf_acf_pre_post_visualizations():
    ...
    if not channel_options:
        st.error("No valid EEG channels were found for the selected feature.")
        return

    default_channel_index = (
        channel_options.index("AF8") if "AF8" in channel_options else 0
    )
    with filter_3:
        selected_channel = st.selectbox(
            "EEG channel",
            ...
            key="bcf_acf_prepost_channel",
        )

    selected = feature_data[
        feature_data["Channel"] == selected_channel
    ].copy().sort_values("Subject")
    if selected.empty:
        st.warning("No paired observations match the selected filters.")
        return
    ...
    metric_1, metric_2, metric_3, metric_4 = st.columns(4)
    metric_1.metric("Paired Subjects", selected["Subject"].nunique())
    metric_2.metric("Increased", increased)
    metric_3.metric("Decreased", decreased)
    metric_4.metric("Mean Change", format_bcf_acf_change(mean_change))
    ...
    paired_figure = build_bcf_acf_pre_post_paired_figure(
        selected, selected_band, selected_feature, selected_channel
    )
    ...
    with chart_1:
        ...
        st.caption(
            "Each line connects the BCF and ACF values from the same subject. "
            "Upward lines indicate an increase; downward lines indicate a decrease."
        )
    with chart_2:
        st.plotly_chart(
            distribution_figure,
        ...
        st.caption(
            "The histogram and rug marks show the spread of ACF-minus-BCF "
            "changes. Values to the right of zero are increases."
        )
    ...
    with chart_3:
        ...
        st.caption(
            "Bars show the mean paired change for the selected feature at each "
            "channel; diamond markers show the corresponding median change."
        )
    with chart_4:
        st.plotly_chart(
            direction_figure,
        ...
        st.caption(
            "Each stacked bar shows the percentage of paired subjects who "
            "increased, decreased, or showed no change at each channel."
        )
    ...
    with st.expander("View selected pre-post data", expanded=False):
        display_columns = [
            "Subject",
            "Channel",
            "Feature",
        ...
        st.download_button(
            "Download selected pre-post data (CSV)",
            data=selected[display_columns].to_csv(index=False).encode("utf-8"),
            file_name=(
                f"BCF_ACF_PrePost_{selected_band}_{safe_feature}_"
                f"{selected_channel}.csv"
            ),
            mime="text/csv",
            key="bcf_acf_prepost_download",
        )

    with st.expander("Visualization and methodological notes", expanded=False):
        st.markdown(
            """
- Change is calculated as ACF minus BCF for every paired subject.
- Mean-change and direction charts compare channels only within the selected band and feature.
- Band Power, Relative Power, Entropy, and Hjorth features use different numerical scales; raw changes should not be compared across different feature types.
- Entropy and the three Hjorth features are broadband measurements repeated in each frequency-band file. Select one band at a time to avoid treating those repetitions as independent observations.
- These visualizations describe direction, magnitude, and heterogeneity. Statistical significance must be evaluated in the Paired Statistics tab.
            """
        )
