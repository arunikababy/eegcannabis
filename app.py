...
def render_subject_10_analysis(comparison_name, test_results, train_results):
    """Render a concise interpretation of one completed comparison."""
    st.subheader(f"Analysis: {comparison_name}")

    if test_results.empty or train_results.empty:
        with st.container(border=True):
            st.subheader("Analysis status")
            st.write("The analysis will appear after the training and test results are available.")
        return

    best_test = test_results.loc[test_results["Accuracy"].idxmax()]
    best_train = train_results.loc[train_results["Accuracy"].idxmax()]
    average_test = test_results["Accuracy"].mean()
    average_train = train_results["Accuracy"].mean()
    test_spread = test_results["Accuracy"].max() - test_results["Accuracy"].min()
    dominant_test_model = test_results["Model"].mode().iloc[0]

    metric_1, metric_2, metric_3, metric_4 = st.columns(4)
    metric_1.metric("Best Test Accuracy", f"{best_test['Accuracy']:.2%}")
    metric_2.metric("Top-5 Test Average", f"{average_test:.2%}")
    metric_3.metric("Top-5 Training Average", f"{average_train:.2%}")
    metric_4.metric("Top-5 Test Spread", f"{test_spread:.2%}")

    analysis_chart = pd.DataFrame(
        {
            "Evaluation Set": ["Training", "Test"],
            "Top-5 Average Accuracy": [average_train, average_test],
        }
    )
    figure = px.bar(
        analysis_chart,
        x="Evaluation Set",
        y="Top-5 Average Accuracy",
        color="Evaluation Set",
        text="Top-5 Average Accuracy",
    ...
    with insight_left:
        with st.container(border=True):
            ...
            st.write(
                f"The strongest test configuration is {best_test['Band']} band with "
                f"{best_test['Model']} at {best_test['Accuracy']:.2%}. "
                f"{dominant_test_model} is the most frequent model among the five highest "
                "test results."
            )
    with insight_right:
        with st.container(border=True):
            st.subheader("Training reference")
            st.write(
                f"The strongest listed training configuration is {best_train['Band']} band "
                f"with {best_train['Model']} at {best_train['Accuracy']:.2%}. The chart compares "
                "the averages of the available Top-5 rankings, not identical model pairs."
            )

    with st.container(border=True):
        st.subheader("Interpretation boundary")
        st.write(
            "These rankings use the Real + Synthetic evaluation data shown in this report. "
            "They describe performance within the current augmented experiment and should be "
            "confirmed using real-only unseen EEG data before making a generalisation claim."
        )


def render_subject_10_pending_analysis(comparison_name):
    """Render the Analysis tab for a comparison that is not ready yet."""
    with st.container(border=True):
        st.subheader(f"Analysis: {comparison_name}")
        st.write(
            "Cross-band performance interpretation will appear here after the training and "
...
def render_subject_10_all_conditions():
    ...
    with statistics_tab:
        st.subheader("Dataset statistics across conditions")
        ...
        chart_data = completed_data[["Comparison", "Training", "Validation", "Test"]].melt(
            id_vars="Comparison",
    ...
    with synthetic_tab:
        st.subheader("Synthetic data coverage")
        quality_rows = [
            {
                "Comparison": "BCF vs ACF",
        ...
        with st.container(border=True):
            ...
            st.write(
                "The available tables confirm balanced one-to-one synthetic augmentation. "
                "Sample counts measure augmentation coverage only; synthetic signal quality "
                "still requires distribution, fidelity, and downstream validation metrics."
            )

    with classification_tab:
        st.subheader("Best classification result by condition")
        classification_display = completed_data[
            ["Comparison", "Best Band", "Best Model", "Best Test Accuracy"]
        ].copy()
        classification_display["Accuracy"] = classification_display["Best Test Accuracy"].map(
            lambda value: f"{value:.2%}"
        )
        st.dataframe(
            classification_display.drop(columns="Best Test Accuracy"),
            use_container_width=True,
            hide_index=True,
        )
