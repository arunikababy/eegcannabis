...
def build_bcf_acf_pre_post_paired_figure(feature_data, band, feature):
    """Build four channel panels of paired BCF-to-ACF trajectories."""
    paired_data = feature_data.melt(
    ...
    figure = px.line(
        ...
        y="Value",
        color="Subject Label",
        line_group="Subject Label",
        markers=True,
        facet_col="Channel",
        facet_col_wrap=2,
        category_orders={
            "Condition": ["BCF", "ACF"],
            "Channel": BCF_ACF_CHANNEL_ORDER,
        },
        title=(
            "Paired Before-After Plot Across EEG Channels<br>"
    ...
    figure.for_each_annotation(
        lambda annotation: annotation.update(
            text=annotation.text.replace("Channel=", "")
        )
    )
    figure.update_xaxes(title_text="")
    figure.update_yaxes(title_text="Feature Value")
    figure.update_layout(
        showlegend=False,
        height=720,
        margin=dict(l=20, r=20, t=75, b=20),
...
def build_bcf_acf_change_distribution_figure(feature_data, band, feature):
    """Build channel-level violin distributions of paired change values."""
    figure = px.violin(
        feature_data,
        x="Channel",
        y="Change",
        ...
        title=(
            "Distribution of Paired Change by EEG Channel<br>"
            f"<sup>{band} | {feature}</sup>"
        ),
    )
    ...
    figure.add_hline(y=0, line_color="#374151", line_dash="dash")
    figure.update_layout(
        showlegend=False,
        height=460,
        margin=dict(l=20, r=20, t=75, b=20),
...
def render_bcf_acf_pre_post_visualizations():
    ...
    with chart_1:
        ...
        st.caption(
            "Violin shapes, box plots, and individual points compare the "
            "distribution of ACF-minus-BCF changes across all four channels."
        )
    with chart_2:
        st.plotly_chart(
            mean_figure,
    ...
    st.caption(
        "Each 100% stacked bar shows the proportion and number of paired "
        "subjects who increased, decreased, or showed no change at each channel."
    )
    ...
    with st.expander("View filtered group-level data", expanded=False):
        display_columns = [
            "Subject",
            "Channel",
            "Feature",
        ...
        st.download_button(
            "Download filtered group-level data (CSV)",
            data=feature_data[display_columns].to_csv(index=False).encode("utf-8"),
            file_name=(
                f"BCF_ACF_PrePost_{selected_band}_{safe_feature}_All_Channels.csv"
            ),
            mime="text/csv",
