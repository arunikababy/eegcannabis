...
def render_landing_page():
    ...
    with third_column:
        render_report_card(
            ...
            "Subject 30",
            "Large-scale cannabis EEG analysis prepared for a comparative study across "
            "30 users, with expanded preprocessing, ACGAN augmentation, and model evaluation.",
            "BCF vs ACF",
            "Explore Report",
            "subject_30",
        )
...
def render_subject_10():
    ...
    st.title("Subject 10: BCF vs ACF")
    st.write(
        "Multi-subject cannabis EEG analysis prepared for a comparative study across "
        "10 users, focusing on Before Cannabis Consumption (BCF) and After Cannabis "
        "Consumption (ACF) conditions."
    )
    ...
    with overview_tab:
        ...
        with left_column:
            with st.container(border=True):
                st.subheader("Research objective")
                st.write("Compare multi-subject cannabis EEG conditions before and after cannabis consumption using machine-learning and deep-learning models.")
        with right_column:
            with st.container(border=True):
                st.subheader("Classification labels")
    ...
    with acgan_tab:
        ...
        with st.container(border=True):
            st.subheader("Augmentation strategy")
            st.write("ACGAN generates a synthetic counterpart for each real sample. The real and synthetic samples are then combined for the classification experiment.")
...
def render_empty_report(subject_name, report_number, user_count):
    ...
    st.title(f"{subject_name}: BCF vs ACF")
    st.write(
        f"This multi-subject cannabis EEG report is prepared for a comparative analysis "
        f"across {user_count} users, focusing on BCF and ACF conditions."
    )
    st.write(
    ...
    with dataset_tab:
        with st.container(border=True):
            ...
            st.write("Dataset files have not been added yet. The BCF and ACF EEG data will appear here.")
    with models_tab:
        with st.container(border=True):
